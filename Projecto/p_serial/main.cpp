...
const double G = 6.67408e-11; // Constante gravitacional

// Estrutura para armazenar as propriedades das partículas
struct Particle {
    double x, y;       // Posições (x, y)
    double velocidade_x, velocidade_y;     // Velocidades (vx, vy)
    double massa;       // Massa
};

// Função para inicializar as partículas
void init_particles(int num_particles, double space_size, vector<Particle>& particles, int seed) {
    srand(seed);

    // Inicializando partículas com uma distribuição uniforme para a posição e velocidade
    for (int i = 0; i < num_particles; i++) {
        Particle p;
        // Posições aleatórias dentro do espaço
        p.x = rand() % int(space_size);
        p.y = rand() % int(space_size);

        // Velocidades aleatórias dentro de um intervalo
        p.velocidade_x = (rand() % 200 - 100) / 100.0; // Velocidade aleatória entre -1.0 e 1.0
        p.velocidade_y = (rand() % 200 - 100) / 100.0;

        // Massa aleatória para cada partícula entre 1 e 100
        p.massa = rand() % 100 + 1;
